...
int main(int argc, char **argv)
{
	...
	if ((port_desc = serialOpen ("/dev/ttyACM0", 115200)) >= 0){
		printf ("ACM0 opened");
  	}
	else if ((port_desc = serialOpen ("/dev/ttyACM1", 115200)) >= 0){
		printf ("ACM1 opened");
	}
	else if ((port_desc = serialOpen ("/dev/ttyACM2", 115200)) >= 0){
		printf ("ACM2 opened");
	}
	else{
		printf ("open failed");
